<template>
  <v-data-table-server
    ...
    :items-length="filteredArticles.length"
    :loading="loading"
  ...
  >
    ...
    <template v-slot:item.story_url="{ item }">
      <v-hover v-slot:default="{ isHovering, props }">
        <v-row
          :style="{
            backgroundColor: isHovering ? 'grey' : 'transparent',
            borderRadius: '50%',
            width: '35px',
            height: '35px',
            display: 'flex',
        ...
        >
          <template v-if="item.story_url">
            ...
            >
              <v-icon v-bind="props" size="35px" color="green"
...
const filteredArticles = computed(() => {
  return props.filter
    ? articles.value.filter((article) => article.story_url)
    : articles.value;
});
